...
- `fct_infreq()`: by number of observations with each level
          (largest first)
...
- `fct_reorder2(x,y)`: by sorting along another 2 variables

- [!] All the above functions return **new factors**.

## Include NA

When a factor contains NAs, NAs will not form a level. And when plotting, NA always orders first. We can use `fct_explicit_na(x, "NA")` to make NA a real level with the name `"NA"`. Then we can reorder the levels including `"NA"`
